...
class MotionHead(BaseMotionHead):
    ...
    @nvtx.annotate("mothion forward", color="purple")
    def forward_train(self,
        ...
        with nvtx.annotate("filter_vehicle_query", color="orange"):
    ...
    def forward(self, 
        ...
        with nvtx.annotate("extract_track_center", color="yellow"):
        ...
        learnable_query_pos = None
        with nvtx.annotate("construct embedding", color="blue"):
        ...
        scene_level_offset_anchors = None
        agent_level_norm = None
        scene_level_ego_norm = None
        scene_level_offset_norm = None
        with nvtx.annotate("construct embedding", color="blue"):
        ...
        with nvtx.annotate("embedding layer", color="red"):
    ...
    @nvtx.annotate("group_mode_query_pos", color="yellow")
    ...
    @nvtx.annotate("motion loss", color="gray")
    def loss(self,
             gt_bboxes_3d,
             gt_fut_traj,
             gt_fut_traj_mask,
        ...
        with nvtx.annotate("for_loop_num_dec_loss", color="orange"):
    ...
    @nvtx.annotate("compute_matched_gt_traj", color="green")
    def compute_matched_gt_traj(self,
        ...
        with nvtx.annotate("for num img loop", color="yellow"):
            for i in range(num_imgs):
                with nvtx.annotate("echo imag cal", color="red"):
                    ...
                    if self.use_nonlinear_optimizer:
                        ...
                        # print("track_bbox_results device :", track_bbox_results[i][0].device)
                        # print("valid_traj_masks device :", valid_traj_masks.device)
                        bboxes = track_bbox_results[i][0].tensor[valid_traj_masks.detach().cpu()]
                        matched_gt_bboxes_3d = gt_bboxes_3d[i][-1].tensor[matched_gt_idx[:-1].detach().cpu()
                                                                        ][valid_traj_masks[:-1].detach().cpu()]
