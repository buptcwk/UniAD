...
class PlanningHeadSingleMode(nn.Module):
    ...
    @nvtx.annotate("planning forward_train", color="blue")
    def forward_train(self,
        ...
        losses = None
        with nvtx.annotate("plan loss", color="orange"):
    ...
    @nvtx.annotate("planning forward", color="green")
    def forward(self, 
                bev_embed, 
                occ_mask, 
                bev_pos, 
        ...
        with nvtx.annotate("mlp", color="green"):
        ...
        ##### Plugin adapter #####
        with nvtx.annotate("adapter", color="yellow"):
        ...
        with nvtx.annotate("attn_module", color="orange"):
            plan_query = self.attn_module(plan_query, bev_feat)   # [1, 1, 256]
        
        sdc_traj_all = None
        with nvtx.annotate("cal_sdc_traj_all", color="green"):
    ...
    @nvtx.annotate("collision_optimization", color="orange")
    ...
    @nvtx.annotate("plan loss func", color="green")
    def loss(self, sdc_planning, sdc_planning_mask, outs_planning, future_gt_bbox=None):
        ...
        loss_dict = dict()
        with nvtx.annotate("for loss collision", color="red"):
            for i in range(len(self.loss_collision)):
                with nvtx.annotate("each loss collision", color="yellow"):
