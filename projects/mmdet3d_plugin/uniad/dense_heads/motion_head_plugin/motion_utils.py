...
@nvtx.annotate("nonlinear_smoother", color="green")
def nonlinear_smoother(gt_bboxes_3d, gt_fut_traj, gt_fut_traj_mask, bbox_tensor):
    ...
    @nvtx.annotate("check diff", color="white")
    ...
    with nvtx.annotate("for gt_fut_traj loop", color="red"):
        for i in range(gt_fut_traj.shape[0]):
            with nvtx.annotate("each gt_fut_traj cal", color="blue"):
                ts = ts_limit[i]
