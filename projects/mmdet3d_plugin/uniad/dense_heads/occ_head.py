...
class OccHead(BaseModule):
    ...
    @nvtx.annotate("occ forward", color="green")
    def forward(self, x, ins_query):
        ...
        with nvtx.annotate("bev_light_proj", color="black"):
            base_state = self.bev_light_proj(base_state)
        with nvtx.annotate("base_downscale", color="orange"):
        ...
        with nvtx.annotate("occ for loop", color="yellow"):
            for i in range(self.n_future_blocks):
                with nvtx.annotate("occ core", color="red"):
                    # Downscale
                    ...
                    with nvtx.annotate("occ transformer", color="black"):
